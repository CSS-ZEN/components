...
export default function Markdown (markdown: IMarkdownProps) {
    ...
    useEffect(() => {
        ...
        const headers = attachAnchors($article.current)
        resetPres($article.current)
        const pres = attachCopyButton($article.current, setClipboard)
...
function resetPres (container: HTMLElement) {
...
function attachAnchors (container: HTMLElement) {
...
function attachCopyButton (container: HTMLElement, setClipboard: (text: string) => void) {
    const pres = container.querySelectorAll('pre')
    pres.forEach(pre => {
